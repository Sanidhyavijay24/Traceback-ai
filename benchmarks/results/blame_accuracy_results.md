...
> **Summary:** **100.0% Top-1 Accuracy** (14/14 failure scenarios correctly attributed) | **0/4 False Positives** on healthy traces | Execution time: **0.13s**
...
|---|---|---|---|---|---|---|
| `retrieval_01_unrelated_chunks` | `retrieval` | `retrieve_docs` | `retrieve_docs` | `1.82` | **PASS** | Retrieved passages had low query similarity (0.00 < 0.33). Downstream st... |
...
| `retrieval_04_weak_retrieval_healthy_llm` | `retrieval` | `search_knowledge_base` | `search_knowledge_base` | `1.82` | **PASS** | Retrieved passages had low query similarity (0.00 < 0.33). Downstream st... |
...
| `llm_06_truncated_output` | `llm` | `generate_architecture_explanation` | `generate_architecture_explanation` | `0.08` | **PASS** | Model response was unusually short (11 tokens). May indicate truncation,... |
...
| `llm_08_multisample_inconsistency` | `llm` | `generate_chemistry_answer` | `generate_chemistry_answer` | `0.04` | **PASS** | Model response was unusually short (18 tokens). May indicate truncation,... |
...
| `cascading_13_upstream_root_cause` | `cascading` | `retrieve_kb_context` | `retrieve_kb_context` | `1.82` | **PASS** | Retrieved passages had low query similarity (0.00 < 0.33). Downstream st... |
...
| `healthy_16_clean_rag` | `healthy` | `(healthy)` | `generate_rag_answer` | `0.03` | **PASS** | LLM response quality sub-scores degraded (overall health score: 0.93). |
| `healthy_17_clean_tool` | `healthy` | `(healthy)` | `format_tax_summary` | `0.03` | **PASS** | LLM response quality sub-scores degraded (overall health score: 0.93). |
| `healthy_18_clean_multistep_agent` | `healthy` | `(healthy)` | `generate_customer_reply` | `0.03` | **PASS** | LLM response quality sub-scores degraded (overall health score: 0.93). |
| `healthy_19_conversational_bm25_query` | `healthy` | `(healthy)` | `retrieve_db_indexing_docs` | `0.05` | **PASS** | Retrieval relevance was healthy (score: 0.89 >= threshold 0.33). |
